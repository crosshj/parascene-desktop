...
**Repo status:** Shell + **Library catalog** + **manifest sync** from Parascene are in place. Next: download media files into `~/Movies/Parascene/Library/`. Shell polish leftovers (About / updates) are tracked separately.

```text
Shell scaffold (done)
  → Library | Project + SQLite + creations manifest sync (done; media download next)
...
| Requirement | Status |
| --- | --- |
| Tauri 2 + React + TypeScript; `npm run dev` / `build` / `test` | Done |
...
| Library \| Project chrome + spacer + context tabs | Done (stubs) — see library plan |
| Local SQLite catalog + paths + Creations/Sync UI | Done |
| Manifest sync (`GET /api/create/images` → SQLite) | Done |
| Media download to `Library/media` + thumbs | Done |
| Resume/checksums + sync policy choices | Next — see library plan |
...
Manifest sync + media download into `Library/media` are in place. Next polish: resume/checksums and sync policy choices per [PLAN-library-sync.md](./PLAN-library-sync.md).
