...
Files live in `integration/*.integration.test.*`, numbered so they run in order: `01` connect, `02` sync, `03` project, `04` folder, `05` generate, `06` audio-to-video, `07` image models, `08` identity, `09` local-only, `10` Assets Remove/Delete, `11` thumbs/media rows, `12` speech/music generate, `13` speech voices. They talk to the live loopback API only. One file at a time.
...
`12` is its own project. Flash TTS on A1, a spoken file on A2 after a gap, Publisher mix proof, then Help shots. Parascene speech/music tiles color from Creation meta the same way BYO Replicate stamps do (FLASH / LYRIA / MM SPEECH / MM MUSIC). Audio models (`audio-models.html`) is Topics only — not this suite. Never voice-train from `generation.audio`.

`13` is its own project. Same spoken line on Gemini and MiniMax, every English MiniMax system id (picker or Custom), plus pause / emotion / sound / Style clips. Publishes Help audio and three form shots. Skip a clip when the Help mp3 is already on disk. Never voice-train from `generation.audio`.
...
- `generation.audio` `{ projectId, intent: text_to_speech | text_to_music, prompt, model?, voice?, voice_id?, emotion?, style?, generate? }` — lands Editor New asset on Parascene server 1 (`replicateSpeech` / `replicateMusic`). Default generates and waits (up to 12 minutes). If the Creation is ready but the file is not on disk yet, it lands Sync and runs Sync media, then returns to Editor. `generate: false` only opens the filled form. MiniMax ids that are not in the Voice list go through Custom (`voice=custom` + `voice_id`). `emotion` is MiniMax. `style` is Gemini. Cheap first-run speech: Gemini Flash TTS (Kore). Live Parascene credits — no BYO Replicate.
- `timeline.place` `{ projectId, assetId, audioTrack?: 1 | 2, startSec? }` — lands Editor and drops that audio asset on A1 or A2. A2 turns on `editorAudio2`. Default start is after the last clip on that track.
...
- `help.open` `{ topicId? }` — open Help in the default browser. Does not require signed-in. Omit topicId for the contents page. Articles are static HTML under `public/help/`. Topic ids: `getting-started` (or `start`, `overview`, `screens`), `projects` (or `create-project` / `open-project`), `folders`, `sync` (or `local-and-cloud` / `this-computer` / `cloud` / `remote`), `generate` (or `generate-image`), `generate-audio` (or `tts` / `text-to-speech` / `generate-music`), `image-models` (or `models`), `audio-models` (or `audio-model` / `speech-models` / `music-models` / `tts-models`), `speech-voices` (or `voices` / `custom-voice` / `voice-id`), `video-models` (or `video-model`), `audio` (or `speech` / `a2v` / `audio-to-video`), `settings` (or `labs`), `tools` (or `local-tools` / `ffmpeg` / `demucs` / `whisper`). Screen jumps: `library`, `director`, `editor` (Getting started headings).
