...
- `generation.audio` `{ projectId, intent: text_to_speech | text_to_music, prompt, model?, voice?, generate? }` — lands Editor New asset on Parascene server 1 (`replicateSpeech` / `replicateMusic`). Default generates and waits (up to 12 minutes). `generate: false` only opens the filled form. Cheap first-run models: Gemini Flash TTS (Kore / Puck) and Lyria 3. Live Parascene credits — no BYO Replicate.
