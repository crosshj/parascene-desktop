...
- `generation.audio` `{ projectId, intent: text_to_speech | text_to_music, prompt, model?, voice?, generate? }` — lands Editor New asset on Parascene server 1 (`replicateSpeech` / `replicateMusic`). Default generates and waits (up to 12 minutes). If the Creation is ready but the file is not on disk yet, it lands Sync and runs Sync media, then returns to Editor. `generate: false` only opens the filled form. Cheap first-run speech: Gemini Flash TTS (Kore). Live Parascene credits — no BYO Replicate.
...
- `help.open` `{ topicId? }` — open Help in the default browser. Does not require signed-in. Omit topicId for the contents page. Articles are static HTML under `public/help/`. Topic ids: `getting-started` (or `start`, `overview`, `screens`), `projects` (or `create-project` / `open-project`), `folders`, `sync` (or `local-and-cloud` / `this-computer` / `cloud` / `remote`), `generate` (or `generate-image`), `generate-audio` (or `tts` / `text-to-speech` / `generate-music`), `image-models` (or `models`), `audio-models` (or `audio-model` / `speech-models` / `music-models` / `tts-models`), `video-models` (or `video-model`), `audio` (or `speech` / `a2v` / `audio-to-video`), `settings` (or `labs`), `tools` (or `local-tools` / `ffmpeg` / `demucs` / `whisper`). Screen jumps: `library`, `director`, `editor` (Getting started headings).
