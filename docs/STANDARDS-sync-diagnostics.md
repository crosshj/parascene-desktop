...
| `folder id already exists` | Pending `create` for an id already on cloud (partial upload / never-acked create). Sync **drops** that create unless a pending `delete` remains **and** the folder still exists locally (release: delete then recreate). Create-then-delete leftovers after an empty local delete are dropped; unfile-moves of missing catalog ids are dropped. Retry Sync folders. | Disk trace phase `drop-redundant-ops`; pending should lose the stale create. |
