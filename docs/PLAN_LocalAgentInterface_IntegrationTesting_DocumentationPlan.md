...
Behavioral truth stays the agent API. `src/help/help.test.ts` locks topic links and the button labels the journeys name. Topic ids: `getting-started` (also `overview` / `screens`), `projects`, `folders`, `local-and-cloud` (this computer vs the cloud), `sync`, `generate`, `audio`, `image-models`, `video-models`, `settings`, `tools`, plus screen jumps `library`, `director`, `editor` on Getting started.
