...
/** Long string fields that should render as a textarea. */
export function isPromptLikeField(field: ReplicateInputField): boolean {
  if (field.typeName !== "string") return false;
  if (field.enumValues?.length) return false;
  if (isAnyFileField(field)) return false;
  return (
